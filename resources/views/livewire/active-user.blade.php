<tbody>
    @foreach ($users as $key => $user)
        <tr>
            ...
            <td>
                @if ($user->blocked_at != NULL)
                    <button class="btn btn-success" wire:click.prevent="activateUser('{{ $user->userID }}')">Activate</button>
                @else
                    <button class="btn btn-danger" wire:click.prevent="deactivateUser('{{ $user->userID }}')">Deactivate</button>
                @endif
        ...
        <div class="modal fade" id="delete{{ $user->userID }}" tabindex="-1"
